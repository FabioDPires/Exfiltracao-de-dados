...
public class UI {
    ...
    public static boolean answer(String ev, String v) {
        ...
        Collection<Evidence> evidences = (Collection<Evidence>) DataExfiltration.KS.getObjects(new ClassObjectFilter(Evidence.class));
        ...
        if (questionFound) {
            if (evidence.getValue().compareTo(v) == 0) {
                DataExfiltration.agendaEventListener.addLhs(evidence);
                return true;
            } else {
                return false;
            }
        ...
        DataExfiltration.KS.insert(e);

        if (value.compareTo(v) == 0) {
            DataExfiltration.agendaEventListener.addLhs(e);
            return true;
        } else {
            return false;
        }
